...
$\mathbf Q^i$ 不由输入图像生成。无论输入是雪天铁路、夜间街道还是历史建筑，同一组 queries 都会以相同的“问题集合”扫描局部特征。输入变化体现在 attention 权重与聚合结果中，而聚合坐标系本身保持一致。
...
这是 query self-attention。它让不同 queries 知道彼此正在关注什么，减少重复关注，并形成互补分工。由于 $\mathbf Q^i$ 与输入无关，这部分在推理时可以预先计算并缓存。
...
如果有 $M$ 个 queries，输出 $\mathbf O^i$ 就包含 $M$ 个聚合结果。每个结果不是 query 参数本身，而是 **query 权重作用于输入 value 后得到的图像内容**。

## 3. 为什么要聚合多个网络层

BoQ 并不只使用最后一个 Encoder 的结果，而是把 $L$ 个 block 的输出拼接：
